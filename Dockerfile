FROM mlocati/php-extension-installer:2.2.6 as php_ext_installer
...
RUN curl -L 'https://download.newrelic.com/php_agent/archive/10.19.0.9/newrelic-php5-10.19.0.9-linux.tar.gz' -o /tmp/newrelic.tar.gz && \
    ...
    sed -i 's~/var/log/newrelic/~/tmp/newrelic/~' /usr/local/etc/php/conf.d/newrelic.ini && mv /var/log/newrelic /tmp/newrelic
